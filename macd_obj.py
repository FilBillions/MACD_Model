...
class MACDTable():
        ...
        # model plot
                plt.plot(x_values, self.df['Cumulative MACD Model Return'], label=f'{self.ticker} MACD Model')
        ...
        # print cumulative return % if not already printed
                print(f"{self.ticker} Cumulative MACD Model Return:", round(self.df['Cumulative MACD Model Return'].iloc[-1], 2))
        ...
        def sharpe(self):
                ...
                model_avg_r = float((np.mean(self.df['MACD Model Return'])))
                model_std = float((np.std(self.df['MACD Model Return'])))
